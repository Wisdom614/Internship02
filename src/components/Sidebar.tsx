...
export default function Sidebar() {
  ...
  </aside><nav aria-label="Mobile navigation" className="fixed inset-x-0 bottom-0 z-30 flex h-[72px] items-center justify-around border-t border-[#dce3dc] bg-white px-1 pb-[env(safe-area-inset-bottom)] shadow-[0_-4px_18px_rgba(23,49,38,.08)] md:hidden">{navigation.map(item => <MobileNavItem key={item.path} {...item} active={location.pathname === item.path} />)}</nav></>;
}
...
function MobileNavItem({ icon: Icon, label, path, active }: { icon: any; label: string; path: string; active: boolean }) { return <Link to={path} className={`flex min-w-0 flex-1 flex-col items-center gap-1 rounded-xl py-2 text-[10px] font-semibold ${active ? 'text-[#173126]' : 'text-[#77847c]'}`}><span className={`grid h-7 w-10 place-items-center rounded-lg ${active ? 'bg-[#d9ff6c]' : ''}`}><Icon size={18} strokeWidth={active ? 2.5 : 2} /></span><span className="truncate">{label}</span></Link>; }
